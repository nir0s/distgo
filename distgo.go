...
func main() {
	...
	app := cli.NewApp()
	app.Name = "distro"
	app.Usage = "Show information on your current distribution"
	app.Version = "0.1.0"
	...
	app.Flags = []cli.Flag{
		cli.BoolFlag{
			Name:        "json, j",
			Usage:       "Output in machine readable format",
			Destination: &isjson,
		},
	}
	...
	app.Action = func(_ *cli.Context) {
		...
		if isjson {
			info, _ := json.MarshalIndent(d.Info(false, false), "", strings.Repeat(" ", 4))
			log.Println(string(info))
		} else {
			log.Printf("Name: %s\n", d.Name(true))
			log.Printf("Version: %s\n", d.Version(true, false))
